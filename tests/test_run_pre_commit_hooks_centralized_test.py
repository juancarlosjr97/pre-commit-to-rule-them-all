...
class TestYamlConfigMethods(unittest.TestCase):
    ...
    @patch('pre_commit_to_rule_them_all.run_pre_commit_hooks_centralized.subprocess.run')
    @patch('pre_commit_to_rule_them_all.run_pre_commit_hooks_centralized.os')
    def test_use_pre_commit_hooks_github_action_static_analysis(self, os_mock, mock_subprocess_run):
        """
        Test a valid path for GitHub Action static analysis pre-commit
        """
        os_mock.path.isdir.return_value = False
        os_mock.path.exists.return_value = True
        mock_subprocess_run.return_value.returncode = 0

        run_pre_commit_hooks_centralized.use_pre_commit_hooks_github_action_static_analysis()
